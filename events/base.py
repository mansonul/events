...
class BaseTheme(object):
    ...
    project_name = "Build your forms"  # Project name
    ...
    def validate_plugin_data(self, form_element_entries, request=None):
        ...
        for form_element_entry in form_element_entries:
            ...
            if plugin.data.name == data['name']:
                self._errors.update({'name': ["Duplicate field name!"]})
                return False
            ...
            if hasattr(plugin.data, 'label') and \
               plugin.data.label == data['label']:
                ...
                self._errors.update({'label': ["Duplicate label name!"]})
                return False
...
class FormHandlerPlugin(BasePlugin):
    ...
    def run_integration_handler(self,
        ...
        return (
            False,
            ("No integration handler for plugin {} found.").format(self.uid)
        )
...
def ensure_autodiscover():
    ...
    """
    if not (form_element_plugin_registry._registry and
            form_handler_plugin_registry._registry and
            theme_registry._registry):
        autodiscover()
...
def get_theme(request=None, theme_uid=None, as_instance=False):
    ...
    if not Theme:
        raise ThemeDoesNotExist(
            ("Theme `{0}` does not exist!").format(theme_uid)
        )
